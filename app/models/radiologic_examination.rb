class RadiologicExamination < ApplicationRecord
  ...
  def to_pdf(pdf, show_user = true)
    pdf.font_size(25){  pdf.table([[ "Radiologic Examination ##{id}"]], :row_colors => ['#D999FF'], :column_widths => [ 523], :cell_style=> {align: :center})}
    user.to_pdf_brief_info(pdf) if show_user
    pdf.table([[" Radiologic Examination "]], :row_colors => ['#D999FF'], :column_widths => [ 523], :cell_style=> {align: :center})
