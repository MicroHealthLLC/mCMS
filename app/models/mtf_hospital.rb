class MtfHospital < ApplicationRecord
  ...
  def to_pdf(pdf, show_user = true)
    pdf.font_size(25){  pdf.table([[ "Mtf Hospital ##{id}"]], :row_colors => ['eeeeee'], :column_widths => [ 523], :cell_style=> {align: :center})}
    user.to_pdf_brief_info(pdf) if show_user
    pdf.table([[" MTF Hospital "]], :row_colors => ['eeeeee'], :column_widths => [ 523], :cell_style=> {align: :center})
